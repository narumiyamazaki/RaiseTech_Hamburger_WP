...
$(function() {
    // 置換の対象とするclass属性。
    var $elem = $('.p-header__image--archive--js--iswitch');
    ...
    function imageSwitch() {
      ...
      $elem.each(function() {
        var $this = $(this);
        // ウィンドウサイズが1024px以上であれば_sptbを_pcに置換する。
        if(windowWidth >= replaceWidth) {
          $this.attr('src', $this.attr('src').replace(sptb, pc));
        // ウィンドウサイズが1024px未満であれば_pcを_sptbに置換する。
        } else {
          $this.attr('src', $this.attr('src').replace(pc, sptb));
        }
      });
    }
